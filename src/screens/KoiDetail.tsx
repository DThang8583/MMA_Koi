...
const KoiDetail: React.FC = () => {
  ...
  const handleCommentSubmit = async () => {
    ...
    if (newRating <= 0 || newRating > 5 || newComment.trim() === "") {
      Alert.alert(
        "Thông báo",
        "Vui lòng nhập đánh giá hợp lệ và nội dung bình luận."
      );
      return;
    }

    const userCommented = koi?.comments.some(
      (comment) => comment.author === "YOUR_USER_ID"
    );
    if (userCommented) {
      Alert.alert(
        "Thông báo",
        "Bạn đã bình luận trước đó. Chỉ cho phép một bình luận cho mỗi cá."
      );
      return;
    }
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.infoContainer}>
        ...
        <Text style={styles.value}>
          {new Date().getFullYear() - koi.yob} tuổi
        </Text>
